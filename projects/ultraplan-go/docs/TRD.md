...
This TRD defines the technical requirements for UltraPlan Go, a production-grade Go CLI that implements the proven UltraPlan workflow. Phase 1 covers study initialization, source analysis, report synthesis, code-reference extraction, resumable orchestration, validation, and operational diagnostics. Phase 2 adds governed project and sprint planning through `plan.md` only.
...
- Executing implementation tasks from sprint plans in Phase 2.
...
- Phase 2 does not invoke implementation execution, smoke, review, issues, or Git mutation.
...
## 18. Project and Sprint Planning Technical Requirements

Phase 2 implements the governed planning side of UltraPlan through `plan.md` only:

```text
study -> select -> distill -> reason -> plan
```

The TypeScript prototype includes planning, execution, smoke, review, and issue-tracking behavior. UltraPlan Go Phase 2 ports only the planning artifact chain. Sprint implementation execution, smoke investigation, review automation, local issue tracking, and Git mutation remain deferred.
...
- No execution, smoke, review, or issue stages may appear in Phase 2 flow state.
...
- no placeholders
- does not invoke implementation execution, smoke, review, issue tracking, or Git mutation as Phase 2 CLI behavior
...
Valid `--to` stages end at `plan`. Values for `execute`, `smoke`, `review`, or `issues` must be rejected as out of scope.

### 18.9 Deferred Technical Requirements

The following remain explicitly deferred:

- implementation execution from `plan.md`
- smoke investigation runs
- automated conformance review
- issue tracking
- Git add/commit/push
- implementation task run state
- sprint execution scheduler
...
UltraPlan has two product sides: (1) studying source repositories/documents and producing validated research artifacts, and (2) applying selected study findings to governed project and sprint planning artifacts. This TRD now covers the study side and the planning side through `plan.md`. Implementation execution, smoke investigation, review automation, issue tracking, and Git mutation remain non-goals until a later requirements revision.
