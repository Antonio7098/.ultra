...
- **Expected Outcome:** Users can initialize studies, run large batches of source analyses, synthesize final reports, extract code citations, inspect study status, and then use selected study evidence to generate governed sprint planning artifacts through `plan.md`.
...
For UltraPlan Go, Phase 2 ports the planning artifact chain from the prototype through `plan.md`. Sprint implementation execution, smoke investigation, review automation, issue tracking, and project-management behavior remain future scope.
...
- **Strategic Fit:** UltraPlan Go first becomes the durable study backbone for architecture research, then applies selected findings to governed sprint planning artifacts. Implementation execution remains a later product phase.
...
- **Product workflows stay product-owned:** Runtime adapters execute prompts; UltraPlan owns study behavior, project catalog behavior, and sprint planning behavior.
...
- Keep study-side workflows compatible with planning-side project and sprint artifacts while deferring sprint execution.
...
- Do not implement sprint execution, smoke investigation, review automation, issue tracking, or automatic Git mutation in Phase 2.
...
- **Planning sprint:** A sprint directory under `projects/<project>/sprints/<slug>` that contains governed planning artifacts through `plan.md`.
...
6. Sprint execution, smoke investigation, review automation, or issue tracking.
...
Phase 2 supports sprint planning commands through `plan.md` only:
...
- `ultraplan sprint <project> <sprint> flow --to <stage>`
  - Runs planning stages from missing/invalid state through the requested stage.
...
The following command families remain deferred: `ultraplan sprint ... execute`, `ultraplan sprint ... smoke`, `ultraplan sprint ... review`, `ultraplan issue ...`, and any automatic Git mutation commands.
...
Phase 2 includes only the planning portion:
...
- Flow state records planning-stage progress, failures, skipped optional area reasoning, and timestamps.
...
- Sprint implementation `.run-state.json`.
...
Deferred future data flow: executing implementation tasks, smoke investigations, review automation, and issue tracking are intentionally excluded from Phase 2.
...
- Stop before implementation execution, smoke, review automation, and issue tracking.
